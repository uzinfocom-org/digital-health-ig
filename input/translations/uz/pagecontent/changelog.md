...
Hepatitis JSON formatidagi tashkilotni ConceptMap yordamida UZ Core Organization resursiga o'girishni ko'rsatuvchi [misol](Organization-xonobod-medical-association.html) qo'shildi.

Hepatitis kodlarini UZ Core terminologiyasiga tarjima qilish uchun ConceptMap qo'shildi: [HepatitisMedicalTypeToOrganizationalStructureCM](ConceptMap-hepatitis-medical-type-to-organizational-structure-cm.html) `type.coding[organizationalStructure]` uchun, [HepatitisLevelTypeToSubordinationGroupCM](ConceptMap-hepatitis-level-type-to-subordination-group-cm.html) `type.coding[subordinationGroup]` uchun va [HepatitisServiceTypeToOrganizationalServiceGroupCM](ConceptMap-hepatitis-service-type-to-organizational-service-group-cm.html) `type.coding[organizationalServiceGroup]` uchun. Hepatitis dan tarjima qilayotgan dasturchilar tegishli bo'limlarni to'ldirish uchun ushbu xaritalardan foydalanishlari kerak.
