...
- Учитывайте согласие пациента. Запросы на чтение выполняются с учётом [Consent](StructureDefinition-uz-core-consent.html) пациента; при отказе в доступе сервер возвращает `403`. Каждый факт доступа регистрируется в [AuditEvent](StructureDefinition-uz-core-audit-event.html).
