...
Добавлено пространство имён идентификаторов документов (doc) в систему идентификаторов.

### Версия 0.4.0

Добавлен профиль UZ Core Goal с соответствующими кодовыми справочниками и наборами значений.

Добавлены профили UZ Core Questionnaire и QuestionnaireResponse с многоязычной поддержкой для переведённых вариантов ответов.

Добавлен тип идентификатора для номера пенсионного удостоверения с поддержкой NamingSystem в ресурсе Patient.

Уточнены требования к силе привязки (binding strength) в руководстве по моделированию.
...
Гражданство пациента теперь использует коды ISO 3166-1 alpha-2 вместо пользовательских кодов МВД.

Исправлена консистентность кода OrganizationalSpecialization (101 → 101.0).

Исправлен русский перевод для I_3 в OrganizationalSubordinationGroup.

Уточнены описания для NomenclatureGroup, OrganizationalStructure и OrganizationalServiceGroup для объяснения их назначения и иерархии.

Добавлено расширение для отслеживания даты прикрепления пациента к управляющей организации.

Добавлен ConceptMap для преобразования кодов стран МВД в коды ISO 3166-1 alpha-2.

Добавлены явные срезы для номеров иностранных паспортов и водительских удостоверений пациентов с поддержкой кодов стран.

Обновлено описание EpisodeOfCare.

Улучшена документация по идентификаторам, включая уточнение ПИНФЛ и переход на двухбуквенные коды стран (alpha-2).

Добавлен раздел по витальным показателям с руководством и примерами.
