...
Диаграмма межкомпонентной архитектуры ресурсов на этой же странице теперь охватывает компоненты "Управление кровью" и "Сестринское дело", а для компонента "Направления" показывает два принадлежащих ему профиля вместо пустого блока.

Эта диаграмма теперь открывается со свёрнутыми компонентами: на каждой карточке видно число её ресурсов, а список ресурсов раскрывается по нажатию на карточку. Все 52 ресурса сразу помещались в рамку лишь при масштабе 40%, из-за чего названия ресурсов было трудно читать; свёрнутый вид помещается при 80%. Пока оба компонента свёрнуты, ресурсные связи между ними отображаются одной линией.
