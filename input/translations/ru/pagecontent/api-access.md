...
Прежде чем отправлять данные на платформу, проверьте их по профилям UZ Core. Используйте [онлайн-валидатор](#validation-web) (ничего не нужно устанавливать) или [командную строку FHIR-валидатора](#validation-cli) (можно автоматизировать, работает офлайн). Оба используют один и тот же движок валидации; важно загрузить это руководство (IG), чтобы профили `uz-core-*` разрешались.

#### Командная строка валидатора {#validation-cli}

Для автоматизации или офлайн-использования скачайте [HL7 FHIR валидатор](https://github.com/hapifhir/org.hl7.fhir.core/releases/latest/download/validator_cli.jar) и выполните:
