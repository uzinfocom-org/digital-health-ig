...
### Прикрепление пациентов к организациям

В системе здравоохранения Узбекистана пациенты прикрепляются к первичной медицинской организации (поликлинике), которая является их основным пунктом оказания медицинской помощи. Это прикрепление фиксируется с помощью двух элементов:

1. **`Patient.managingOrganization`** — ссылка на [UZCoreOrganization](StructureDefinition-UZCoreOrganization.html), к которой прикреплён пациент
2. **Расширение `managingOrganizationAttachment`** — дата прикрепления пациента к данной организации

В Узбекистане пациенты могут менять свою медицинскую организацию только один раз в год. Дата прикрепления используется для соблюдения этого правила.
...
```jsonc
{
  ...
  "extension": [{
    ...
    "valueDate": "2024-03-15"    // Дата прикрепления пациента
  }],
...
При обновлении прикрепления пациента к медицинской организации системы должны:
1. Проверить существующую дату в `managingOrganizationAttachment`
2. Убедиться, что с момента последнего прикрепления прошёл как минимум один год
3. Обновить одновременно `managingOrganization` и дату в расширении
