...
[UZ Core PlanDefinition](StructureDefinition-uz-core-plandefinition.html) now requires a second `useContext` stating the kind of schedule, bound (extensible) to the new [immunization schedule type](ValueSet-immunization-schedule-type-vs.html) value set. The choice-type slicing on `action.definition[x]` has been removed, because it made the validator reject `definitionCanonical`, and the examples now use `definitionCanonical`.
...
Added [UZ Core PlanDefinition](StructureDefinition-uz-core-plandefinition.html) profile for national immunization schedules.
