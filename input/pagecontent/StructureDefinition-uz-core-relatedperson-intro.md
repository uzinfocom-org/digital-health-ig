...
`telecom.system` and `telecom.use` each use a **required** binding, as does `relationship` - the value must come from the bound value set (the Snapshot view above lists each one). When `gender` is `other`, supply the national `gender-other` extension; that extension is only allowed in that case.
