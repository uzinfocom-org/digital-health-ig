...
CodeSystems с более чем 100 кодами заметно замедляют каждый запуск SUSHI, так как FSH перепарсивается при каждой сборке. Для таких CodeSystems выберите один из подходов:

1. **Предгенерация JSON (рекомендуется для статичных справочников).** Соберите IG один раз через SUSHI, затем переместите сгенерированный `fsh-generated/resources/CodeSystem-<id>.json` в `input/vocabulary/`, а исходный `.fsh` файл — в `input/manual-fsh/`. IG Publisher продолжит видеть ресурс через `input/vocabulary/`, а SUSHI больше не будет тратить время на парсинг. Исходный FSH сохраняется в `input/manual-fsh/` для прослеживаемости и для повторной генерации при обновлении справочника. При обновлении: верните `.fsh` обратно в `input/fsh/terminology/`, пересоберите, и снова переместите JSON и FSH.

2. **Ссылка на внешний терминологический сервис.** Если справочник уже доступен на национальном или международном терминологическом сервере (например, СНОМЕД, ЛОИНК, локальный сервер Минздрава), не определяйте CodeSystem локально вообще — ссылайтесь на канонический URL напрямую в ValueSet.

3. **Оставить как FSH.** Подходит для активно редактируемых справочников, где удобство правки важнее времени сборки. Применимо для CodeSystems до ~200 кодов; выше — серьёзно рассматривайте вариант 1.

Файлы в `input/manual-fsh/` SUSHI не обрабатывает — это служит «архивом» исходников для предгенерированных JSON.
